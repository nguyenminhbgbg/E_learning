        ...
        {isLoading ? <ActivityIndicator/> : (
          ...
          {/* img title past */}
            ...
            <View style={{flexDirection:'row', alignContent:'center', alignItems:'center'}} >
              {/* List questions left */}
              <View style={{flexDirection:'column', maxWidth:'45%', minWidth:'44%'}}>
                <FlatList
                data={data}
                keyExtractor={({ id }, index) => id}
                renderItem={({ item }) => (
                  <View>
                    <Text style={styles.textQestion}> {item.noidung_cauhoi}</Text>
                  </View>
                )}
                />
              </View>
              {/* List document right*/}
              <View style={{flexDirection:'column', maxWidth:'50%', minWidth:'49%'}}>
                  <FlatList
                    data={answers}
                    keyExtractor={({ id }, index) => id}
                    renderItem={({ item }) => (
